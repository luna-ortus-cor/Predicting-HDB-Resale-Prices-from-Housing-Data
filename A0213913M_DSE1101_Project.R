...
#KDE
plot(kde(eval(parse(text=paste('data_clean$',TARGET))), h = hlscv(eval(parse(text=paste('data_clean$',TARGET))))), main = paste(TARGET,' KDE - min MISE'), xlab = paste(TARGET))
fname=paste(paste("KDE",TARGET,sep="-"),".pdf",sep="")
pdf(fname)
plot(kde(eval(parse(text=paste('data_clean$',TARGET))), h = hlscv(eval(parse(text=paste('data_clean$',TARGET))))), main = paste(TARGET,' KDE - min MISE'), xlab = paste(TARGET))
dev.off()
for (x in numvars){
...
for (z in 1:length(sets)){
  for (ii in kt){
    ...
    for (x in var_to_use){
      plot(eval(parse(text=paste0('train_',sets[z],'$',x,collapse=''))), eval(parse(text=paste0('train_',sets[z],'$',TARGET,collapse=''))), main = paste("K = ",k), xlab=paste(x), ylab=paste(TARGET), type="p", col=rainbow(k)[grp$cluster])
      fname=paste(paste("kmeans",x,z,sep="-"),".pdf",sep="")
      pdf(fname)
      plot(eval(parse(text=paste0('train_',sets[z],'$',x,collapse=''))), eval(parse(text=paste0('train_',sets[z],'$',TARGET,collapse=''))), main = paste("K = ",k), xlab=paste(x), ylab=paste(TARGET), type="p", col=rainbow(k)[grp$cluster])
      dev.off()
    }
